...
newTaskForm.addEventListener('submit', e => {//get event when you sibmit
  e.preventDefault()
  const taskName = newTaskInput.value//name that you typed in new task
  //check if the new Task or if the Task name
  if (taskName == null || taskName === '') return
  const task = createTask(taskName)
  newTaskInput.value = null
  const selectedList = lists.find(list => list.id === selectedListId)
  selectedList.tasks.push(task)
  saveAndRender()// add new task that type something in ...
})
...
function clearElement(element) {
  while (element.firstChild) {//remove it if the element has a first chilld
